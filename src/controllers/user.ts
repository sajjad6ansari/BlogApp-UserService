...
//Get My Profile
export const myProfile=TryCatchHandler(async(req:AuthenticatedRequest,res)=>{
  const user = req.user;
  if (!user) {
...
//Get User Profile
export const getUserProfile=TryCatchHandler(async(req:AuthenticatedRequest,res:Response)=>{
...
//Update User Profile
export const UpdateUserProfile=TryCatchHandler(async(req:AuthenticatedRequest,res:Response)=>{
  if(!req.body){
...
export const updateProfilePicture = TryCatchHandler(async (req: AuthenticatedRequest, res: Response) => {
  const file = req.file;
  if (!file) {
